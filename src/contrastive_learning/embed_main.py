...
def main(
    ...
    """
    # Select the concrete embedder class based on the provided identifier and instantiate it.
    embedder_cls = embeddings.EMBEDDING_MODEL_CLASSES[embedder_type]
    embedder: embeddings.Embedder = embedder_cls()
    ...
    text_embeddings = embeddings.author_infos_to_embeddings(
        authors_by_arm, embedder, embedding_strategy, persona_chunk_size, batch_size
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Number of personas to embed in each chunk. Note: the reason for this is explained in "
        "AveragePostsStrategy.compute_authors_embeddings. In short, chunking the personas leads to faster runtime.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    main(args.embedder, args.output, args.train_file, args.val_file, args.persona_chunk_size, args.batch_size)
